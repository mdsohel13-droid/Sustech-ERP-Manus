...
- **Finance Module**: Comprehensive financial management with dedicated tables for accounts, journal entries, receivables, payables, income/expenditure, and daily sales. Features include KPI dashboards, balance sheets, cash flow analysis, aging reports, and forecasting. Enhanced with AR/AP approval workflow (manager-only approve/reject with rejection reasons), payment recording with partial payment support and auto-status updates (pending→partial→paid), and Budget Management tab with variance analysis comparing budgets vs actuals with visual charts and status indicators.
